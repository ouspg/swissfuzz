...
class SctpEndpoint(Endpoint):
    ...
    def handle_read(self):
        ...
        params = {'fromaddr': fromaddr, 'flags': flags,
            ...
            if server:
                ...
                try:
                    ...
                    sock = ssl.wrap_socket(
                        sock, keyfile=key, certfile=key, ssl_version=ssl.PROTOCOL_SSLv23, server_side=True)
                except ssl.SSLError, e:
                    print e
            else:
                try:
                    sock = ssl.wrap_socket(
                        sock, ssl_version=ssl.PROTOCOL_SSLv23, ciphers="ALL")
                except ssl.SSLError, e:
                    print e
        return sock
...
class Client(NetworkDispatcher):
    ...
    def __init__(self, config):
        ...
        self.server = None
        self.server_sock = create_bound_socket(
            config.proto_num, config.bind_address, True)
        ...
        self.server = self.NetworkEndpoint(
            self.server_sock, config=self.config)
