...
class TShark:
    ...
    def packetxmlparse(self, r, w, packet):
        print 1
        w.write(struct.pack("IIII", packet.time[0], packet.time[
                1], len(packet.data), len(packet.data)))
        w.write(packet.data)
        w.flush()
        lines = ""
        while True:
...
#        return self.psmltshark[linktype]
    ...
    def spawnpdml(self, linktype):
        p = subprocess.Popen(["strace", "-tt", "-o", "foo.trace", self.tshark_path, "-Tpdml", "-n",
                              "-l", "-i-"], bufsize=0, stdin=subprocess.PIPE, stdout=subprocess.PIPE, close_fds=False)
